 ...
 * @param minutes 분
 */
function getNextTriggerTimestamp(jsDay: number, hours: number, minutes: number): number {
  const now = new Date();
...
/** 스케줄의 모든 요일별 알림 등록 */
export async function registerScheduleNotifications(schedule: Schedule): Promise<void> {
...
/** 취소 후 재등록 */
export async function syncScheduleNotifications(schedule: Schedule): Promise<void> {
...
/** 전체 알림 초기화 (앱 시작 시) */
export async function syncAllNotifications(timetables: Timetable[]): Promise<void> {
